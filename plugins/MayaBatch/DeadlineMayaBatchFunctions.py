...
# typing is added in Deadline 10.1.16
try:
...
def getRenderLayerDisplayName(layer_name):
    # type: (str) -> str
    """A method mimicking the built-in mel function: 'renderLayerDisplayName', but first tries to see if it exists"""
...
def unlockRenderSetupOverrides(remove_overrides_json_string):
    ...
    :param remove_overrides_json_string: a json string consisting of a node as a key, with a list of attributes
                                         we want to unlock as the value.
    :return:
    """
    try:
        ...
        for collection in render_layer.getCollections():
            if type(collection) == maya.app.renderSetup.model.collection.RenderSettingsCollection:
                for override in collection.getOverrides():
                    if override.targetNodeName() in remove_overrides and override.attributeName() in remove_overrides[
                        override.targetNodeName()]:
...
def performArnoldPathmapping(startFrame, endFrame, tempLocation=None):
    ...
    """
    if tempLocation:
        performArnoldPathmapping.tempLocation = tempLocation  # type: ignore
    else:
        if not performArnoldPathmapping.tempLocation:  # type: ignore
    ...
    for standIn in standInObjects:
        try:
            ...
            origDir, origFileName = performArnoldPathmapping.originalProperties[standIn]  # type: ignore
        except KeyError:
            ...
            standinTempLocation = performArnoldPathmapping.mappedTempLocations.get(origDir, None)  # type: ignore
            if not standinTempLocation:
                # Generate a random dir for the mapped file, and update our mapping
                standinTempLocation = os.path.join(performArnoldPathmapping.tempLocation, str(uuid.uuid4()))  # type: ignore
                performArnoldPathmapping.mappedTempLocations[origDir] = standinTempLocation  # type: ignore
            ...
            if not os.path.isdir(standinTempLocation):
                ...
                performArnoldPathmapping.originalProperties[standIn] = (origDir, origFileName)  # type: ignore
        ...
        for frame in range(startFrame, endFrame + 1):
            ...
            if not (origDir, fileNameWithFrame) in performArnoldPathmapping.mappedFiles:  # type: ignore
                mapped_dir = performArnoldPathmapping.mappedTempLocations[origDir]  # type: ignore
                # Perform pathmapping
                runPathmappingOnFile(
                    os.path.join(origDir, fileNameWithFrame),
                    os.path.join(mapped_dir, fileNameWithFrame)
                )
                performArnoldPathmapping.mappedFiles.add((origDir, fileNameWithFrame))  # type: ignore


performArnoldPathmapping.tempLocation = ""  # type: ignore
# State property which contains mappings of standin objects to their original fileproperties
performArnoldPathmapping.originalProperties = {}  # type: ignore
...
performArnoldPathmapping.mappedFiles = set()  # type: ignore
# Mapping of orig dirs -> temp locations
performArnoldPathmapping.mappedTempLocations = {}  # type: ignore
...
def CallDeadlineCommand(arguments, hideWindow=True):
    ...
    if os.name == 'nt':
        if hideWindow:
            ...
            if hasattr(subprocess, '_subprocess') and hasattr(subprocess._subprocess, 'STARTF_USESHOWWINDOW'):  # type: ignore
                startupinfo = subprocess.STARTUPINFO()  # type: ignore
                startupinfo.dwFlags |= subprocess._subprocess.STARTF_USESHOWWINDOW  # type: ignore
    ...
    # 2/3 safe manner in which to ensure we get a 'str' back from deadlinecommand
...
def ForceLoadPlugins(*args):
    ...
    :param args: additional plugins to force load
    :return:
    """
    ...
    print("Force loading Maya plugins")

    for plugin in chain(PLUGINS_TO_LOAD, args):
