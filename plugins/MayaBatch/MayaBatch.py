...
from Deadline.Plugins import DeadlinePlugin, PluginType # pylint: disable=import-error, no-name-in-module
from Deadline.Scripting import RepositoryUtils, SystemUtils, StringUtils, FileUtils, PathUtils # pylint: disable=import-error, no-name-in-module
from FranticX.Processes import ManagedProcess # pylint: disable=import-error, no-name-in-module
from System import Environment # pylint: disable=import-error, no-name-in-module
from System.Diagnostics import ProcessPriorityClass # pylint: disable=import-error, no-name-in-module
from System.IO import Path, File, Directory # pylint: disable=import-error, no-name-in-module
from System.Text import StringBuilder # pylint: disable=import-error, no-name-in-module
...
def GetDeadlinePlugin():
    """
    The function that Deadline calls to get an instance of the main DeadlinePlugin class
    """
...
class MayaBatchPlugin(DeadlinePlugin):
    """
    The main DeadlinePlugin class for the MayaBatch plugin
    """
    ...
    def __init__( self ):
        self.InitializeProcessCallback += self.InitializeProcess # pylint: disable=no-member
        self.StartJobCallback += self.StartJob # pylint: disable=no-member
        self.RenderTasksCallback += self.RenderTasks # pylint: disable=no-member
        self.EndJobCallback += self.EndJob # pylint: disable=no-member

        self.scriptContents = ""
        self.isScriptLogged = False

    def Cleanup(self):
        del self.InitializeProcessCallback # pylint: disable=no-member
        del self.StartJobCallback # pylint: disable=no-member
        del self.RenderTasksCallback # pylint: disable=no-member
        del self.EndJobCallback # pylint: disable=no-member
    ...
    def StartJob( self ):
        ...
        if self.EnablePathMapping:
            ...
            self.EnableOpenColorIO = self.GetBooleanPluginInfoEntryWithDefault( "EnableOpenColorIO", False )
        ...
        if self.EnablePathMapping and not self.DirmapPathMapping and os.path.splitext( sceneFilename )[1].lower() == ".ma":
            self.LogInfo( "Performing path mapping on ma scene file" )

            tempSceneFileName = Path.GetFileName( sceneFilename )
            self.SceneFile = os.path.join( self.TempThreadDirectory, tempSceneFileName )
        ...
        if self.StartupScriptPath:
            if not os.path.isfile( self.StartupScriptPath ):
                self.FailRender( "Startup script \"%s\" does not exist." % self.StartupScriptPath )
        ...
        if SystemUtils.IsRunningOnMac():
            ...
            python_version_folder = self._get_maya_python_version_folder(mayaFrameworksFolder)
            if python_version_folder:
                self.SetEnvironmentAndLogInfo("PYTHONHOME", python_version_folder)
    ...
    def _get_maya_python_version_folder(self, framework_folder):
        # type: (str) -> Optional[str]
        """Given the path to the Maya's macOS framework folder, attempts to find the versioned python folder.

        :param framework_folder: path to maya's framework folder
        :return: The path to Maya's python folder, or None if it doesn't exist
        """
        # No point in running this if we're not on macOS
        if not SystemUtils.IsRunningOnMac():
            return None

        versions_folder = os.path.join(framework_folder, "Python.framework", "Versions")
        if self.Version >= 2023:
            ...
            python_version = "2.7"
            python_folder = os.path.join(versions_folder, python_version)
            # Not sure when Maya removed python 2.6, probably very ancient (it was there in 2013)
            if not os.path.isdir(python_folder):
                python_version = "2.6"
                python_folder = os.path.join(versions_folder, python_version)

        if not os.path.isdir(python_folder):
            self.LogWarning("Unable to locate Maya's versioned python folder in: '{}'".format(framework_folder))
            return None
    ...
    def WriteBatchScriptFile( self, scriptBuilder ):
        ...
        if SystemUtils.IsRunningOnWindows():
            with open( outScriptFilename, 'wb' ) as f:
                f.write( self.scriptContents.encode( "mbcs" ) ) # On Windows, write it out as binary single-bytes per string using the current character set. This ensures that the resulting mel file doesn't have unicode characters, because maya will not be able to decode those characters correctly.
        else:
            File.WriteAllText( outScriptFilename, self.scriptContents ) # Non-Windows, we can write it out like this and the unicode characters will be written, but Maya will be able to handle the unicode encoded file.
    ...
    def WriteScriptToLog( self, isError=False ):
        """
        Prints the batch script, that performs the rendering inside Maya, to the render log.
        This will either be triggered by a plugin configuration option or by encountering a fatal error.
        ...
        if isError:
            logFunction( "Encountered an error, logging render script contents for debugging:" )
        else:
            logFunction( "Logging render script contents:" )

        logFunction( "################################ SCRIPT LOG START ################################" )
        logFunction( self.scriptContents.replace( "\r", "" ) )
        logFunction( "################################# SCRIPT LOG END #################################" )
    ...
    def RenderTasks( self ):
        ...
        if self.FirstTask:
            mayaBatchUtilsScriptFilename = os.path.join( self.GetPluginDirectory(), "MayaBatchUtils.mel" )
            self.LogInfo( "Importing Maya Batch Utils melscript: " + mayaBatchUtilsScriptFilename )
            self.FlushMonitoredManagedProcessStdout( self.ProcessName )
            self.WriteStdinToMonitoredManagedProcess( self.ProcessName, 'eval( "source \\"' + mayaBatchUtilsScriptFilename.replace( "\\", "/" ) + '\\";" )' )

            # Wait until eval is complete
            self.LogInfo( "Waiting for Maya Batch Utils melscript to finish loading" )
            self.WaitForProcess()

            self.LogInfo( "Importing Maya Batch Functions python script: %s" % os.path.join( self.GetPluginDirectory(), 'DeadlineMayaBatchFunctions.py' ) )
            ...
            self.WriteStdinToMonitoredManagedProcess( self.ProcessName, importFunctions )

            # Wait until import is complete.
            self.LogInfo( "Waiting for Maya Batch Functions python script to finish importing" )
            self.WaitForProcess()
        ...
        if self.ScriptJob:
            ...
            for key, value in six.iteritems(pluginInfo):
            ...
            if self.Version == 2015:
                ...
                scriptBuilder.AppendLine( '        error( "Invalid scene - there are no containers bifrostContainers." );')
                scriptBuilder.AppendLine( '    // end if')
                scriptBuilder.AppendLine()
            ...
            if self.FirstTask and self.GetBooleanPluginInfoEntryWithDefault( "UseLocalAssetCaching", False ):
                assetToolsScriptFilename = os.path.join( self.GetPluginDirectory(), "AssetTools.mel" )
            ...
            if self.EnablePathMapping:
                if self.FirstTask:
                    if self.DirmapPathMapping: #this check seems to be equivalent to self.DelayLoadScene
                        self.CreateDelayLoadSceneMelscript( scriptBuilder )

                    if self.XGenPathMapping:
                        mappings = [ mappingPair for mappingPair in RepositoryUtils.GetPathMappings() if mappingPair[1] ]
                        if len( mappings ) > 0:
                            scriptBuilder.AppendLine()
                            scriptBuilder.AppendLine('string $deadlineMappings[] = { "' + '", "'.join( mappingComponent.replace("\\", "/") for mappingPair in mappings for mappingComponent in mappingPair) + '" };')
                            scriptBuilder.AppendLine( 'print("------[ XGen path mapping STARTED ]------\\n");' )
                            scriptBuilder.AppendLine( 'if( catch( mapXGen( $deadlineMappings ) ) ) {' )
                            scriptBuilder.AppendLine( 'print("------[ XGen path mapping FAILED ]------\\n");' )
                            scriptBuilder.AppendLine( '} else {' )
                            scriptBuilder.AppendLine( 'print("------[ XGen path mapping SUCCEEDED ]------\\n");' )
                            scriptBuilder.AppendLine( '}' )
                            scriptBuilder.AppendLine()
        ...
        if os.path.splitext(self.ScriptFilename)[1].lower() == ".py":
            script = self.ScriptFilename.replace("\\", "/")
            # py2 / 3 compatible execfile
            melscript = 'eval( python("with open(\'{0}\') as f: exec(compile(f.read(), \'{1}\', \'exec\'), globals(), locals())"));'.format(script, script)
            self.WriteStdinToMonitoredManagedProcess(self.ProcessName, melscript)
        else:
    ...
    def SetRedshiftPathmappingEnv( self ):
        ...
        if self.Renderer == "redshift" and self.EnablePathMapping:
            ...
            newRSMappingFileName = os.path.join( self.TempThreadDirectory, "RSMapping.txt" )
            ...
            self.SetEnvironmentAndLogInfo("REDSHIFT_PATHOVERRIDE_FILE", newRSMappingFileName)
        ...
        for mapping_pair in RepositoryUtils.GetPathMappings():
            ...
            if all(map(valid_mapping_re.match, mapping_pair)):
                valid_mappings.append(
                    [x.replace("\\", "/") for x in mapping_pair]
                )
    ...
    def build_dirmap_commands(self):
        ...
        if not mappings:
            self.LogInfo("No valid path mappings found. Skipping adding dirmap commands.")
            return []
        ...
        for original, mapped in mappings:
            self.LogInfo('Adding dirmap command to map "{}" to "{}"'.format(original, mapped))
    ...
    def CreateDelayLoadSceneMelscript(self, scriptBuilder):
        ...
        pluginsToForceLoad = []  # AbcImport and fbxmaya are loaded by default
        if self.Renderer.startswith('redshift') and self.Version >= 2019:
            # redshift4maya plugin requires xgen but cannot load it properly
            pluginsToForceLoad.append('xgenToolkit')
        # format the parameters to be properly passed through Mel script
        plugins = ', '.join(['\\"{plugin}\\"'.format(plugin=p) for p in pluginsToForceLoad])
        scriptBuilder.AppendLine(
            'catch( python( "DeadlineMayaBatchFunctions.ForceLoadPlugins({plugins})" ) );'.format(plugins=plugins)
        )

        # Use "catchQuiet" to silence errors when loading the scene
        useCatchQuiet = self.GetBooleanConfigEntryWithDefault("SilenceSceneLoadErrors", False)
        ...
        scriptBuilder.AppendLine('int $loadFailed = %s( `file -o $sceneName` );' %
                                 ('catchQuiet' if useCatchQuiet else 'catch'))

        scriptBuilder.AppendLine('if (!$loadFailed) {')

        # show loaded plugins
        scriptBuilder.AppendLine( 'catch( python( "DeadlineMayaBatchFunctions.OutputPluginVersions()" ) );' )
        ...
        if self.StartupScriptPath != "":
            ...
            scriptBuilder.AppendLine( 'if( catch(eval( "source \\"" + $startupScriptName + "\\";" )) ){' )
            scriptBuilder.AppendLine( 'print ( "An error occurred during the Startup Script.\\n" );' )
            scriptBuilder.AppendLine( '}' )
    ...
    def EndJob( self ):
        ...
        if self.EnablePathMapping and not self.DirmapPathMapping and os.path.splitext( self.SceneFile )[1].lower() == ".ma":
    ...
    def unlockRedshiftRenderSetupOverrides( self ):
        ...
        if self.StartFrame or self.EndFrame:
            ...
            if self.StartFrame:
                renderSetupOverrides[ 'defaultRenderGlobals' ].append( 'startFrame' )
            if self.EndFrame:
        ...
        if self.RenumberFrameStart:
            renderSetupOverrides[ 'defaultRenderGlobals' ].append( 'modifyExtension' )
            renderSetupOverrides[ 'defaultRenderGlobals' ].append( 'startExtension' )

        if self.ImagePrefix:
            renderSetupOverrides[ 'defaultRenderGlobals' ].append( 'imageFilePrefix' )
    ...
    def GetInitializeCommand( self ):
        if self.Renderer == "file" or ( self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
            ...
            # RenderMan uses texture files of its own format (.tex) for rendering, instead of original assets (PNGs, JPEGs, EXRs, etc.).
            # These texture files are later used in the final render.
            # If RenderMan can't find the texture files in (.tex) format, it'll successfully render without the assets, but the output will be incorrect.
            # We use txmake argument to automatically convert all textures into RenderMan's .tex format upfront, so customers don't have to convert their assets manually.
            # See https://rmanwiki.pixar.com/display/REN/txmake for more details.
    ...
    def LocalAssetCachingCommand( self ):
        ...
        cacheDirectory = self.GetConfigEntryWithDefault( cacheDirectoryConfigEntry, '' ).strip()
        if not cacheDirectory:
            self.LogWarning( "Disabling local asset caching: The local asset cache directory for this platform is not set. You must set this option in the MayaBatch repository plugin settings." )
            return ""
        ...
        if not os.path.exists( cacheDirectory ):
            try:
                ...
                if not os.path.exists( cacheDirectory ):
                    self.FailRender( 'Failed to create local asset cache directory: "%s". You can modify this directory in the MayaBatch repository plugin settings.' % cacheDirectory )

        networkPrefixes = self.GetConfigEntryWithDefault( "RemoteAssetPaths", "//;X:;Y:;Z:" ).replace( "\\", "/" )
    ...
    def GetStartFrameCommand( self ):
        if len( self.StartFrame ) > 0:
            if self.Renderer == "file" or ( self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
                ...
                return 'removeRenderLayerAdjustmentAndUnlock .animation; setAttr .animation 1; removeRenderLayerAdjustmentAndUnlock .startFrame; setAttr .startFrame ' + self.StartFrame + ';'
            elif self.Renderer in { "mayavector", "mayahardware", "mayahardware2", "turtle", "arnold", "arnoldexport", "redshift", "redshiftexport", "vray", "vrayexport", "ifmirayphotoreal", "renderman22", "renderman22export" }:
    ...
    def GetByFrameCommand( self ):
        if len( self.ByFrame ) > 0:
            if self.Renderer == "file" or ( self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
                ...
                return 'removeRenderLayerAdjustmentAndUnlock .byFrameStep; catch(`setAttr .byFrameStep ' + self.ByFrame + '`);'
            elif self.Renderer in { "mayavector", "mayahardware", "mayahardware2", "renderman", "rendermanris", "rendermanexport", "renderman22", "renderman22export", "turtle", "maxwell", "maxwellexport", "arnold", "arnoldexport", "redshift", "redshiftexport", "ifmirayphotoreal"}:
    ...
    def GetImagePrefixCommand( self ):
        ...
        if len( self.ImagePrefix ) > 0:
            ...
            if self.Renderer == "file" or ( self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
                ...
                redshiftData = [
                    "if( $redshiftMajorVersion >1.2 || ( $redshiftMajorVersion == 1.2 && (int)$redshiftVersions[2] > 81) ){ removeRenderLayerAdjustmentAndUnlock defaultRenderGlobals.imageFilePrefix; catch(`setAttr -type \"string\" defaultRenderGlobals.imageFilePrefix \"" + self.ImagePrefix + "\"`); }",
                ...
                return 'removeRenderLayerAdjustmentAndUnlock rmanGlobals.imageFileFormat; catch(`setAttr -type "string" rmanGlobals.imageFileFormat "' + self.ImagePrefix + '"`);'
            elif self.Renderer == "vray" or self.Renderer == "vrayexport":
    ...
    def GetRenderDirectoryCommand( self ):
        if len( self.CurrentRenderDirectory ) > 0:
            ...
            if self.Renderer == "file" or ( self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
                ...
                return self.GetRedshiftRenderDirectory(' "' + self.CurrentRenderDirectory + '" + "/redshift/" ')
                ...
                return 'removeRenderLayerAdjustmentAndUnlock rmanGlobals.imageOutputDir; catch(`setAttr -type "string" rmanGlobals.imageOutputDir "' + self.CurrentRenderDirectory + '"`);'
            elif self.Renderer == "gelato":
                return 'workspace ' + flags + ' "images" "' + self.CurrentRenderDirectory + '";'
            else:
                return 'workspace ' + flags + ' "images" "' + self.CurrentRenderDirectory + '"; workspace ' + flags + ' "depth" "' + self.CurrentRenderDirectory + '";'
        else:
            if self.Renderer == 'redshiftexport':
                return self.GetRedshiftRenderDirectory(' `workspace -q -rd` + "redshift/" ')

        return ''

    def GetRedshiftRenderDirectory( self, sceneName ):
        """
        Returns a string, that contains a mel script for getting a render directory for Redshift export job.
        """
    ...
    def GetCameraCommand( self ):
        if len( self.Camera ) > 0:
            ...
            elif self.Renderer in { "mayasoftware", "mentalray", "mayavector", "mayahardware", "mayahardware2", "renderman", "rendermanris", "renderman22", "turtle", "gelato", "mentalrayexport", "finalrender", "maxwell", "maxwellexport", "mayakrakatoa", "arnold", "arnoldexport", "octanerender", "causticvisualizer", "redshift", "redshiftexport", "ifmirayphotoreal"}:
    ...
    def GetResolutionCommand( self ):
        if len( self.Width ) > 0 and len( self.Height ) > 0:
            if self.Renderer == "renderman" or self.Renderer == "rendermanris" or self.Renderer == "rendermanexport":
                ...
                return '$options += " -resolution %s %s";' % ( self.Width, self.Height )
        return ''
    ...
    def GetRenderLayerCommand( self ):
        if len( self.RenderLayer ) > 0:
            ...
            elif self.Renderer in {"mayasoftware", "mentalray", "mayavector", "mayahardware", "mayahardware2", "renderman", "rendermanris", "turtle", "gelato", "vray", "mentalrayexport", "finalrender", "mayakrakatoa", "arnold", "arnoldexport", "octanerender", "causticvisualizer", "redshift", "ifmirayphotoreal"}:
                return '$rl="' + self.RenderLayer + '";'
            elif self.Renderer == "redshiftexport":
    ...
    def GetThreadsCommand( self ):
        if len( self.Threads ) > 0:
            if self.Renderer in [ "mayasoftware" ]:
                ...
                numThreads = int( self.Threads )
                if numThreads > 0:
                    return 'removeRenderLayerAdjustmentAndUnlock defaultArnoldRenderOptions.threads_autodetect; catch(`setAttr defaultArnoldRenderOptions.threads_autodetect 0`);removeRenderLayerAdjustmentAndUnlock defaultArnoldRenderOptions.threads;catch(`setAttr defaultArnoldRenderOptions.threads ' + self.Threads + '`);'
                else:
    ...
    def GetRegionCommand( self ):
        if self.RegionRendering:
            if len( self.Left ) > 0 and len( self.Right ) > 0 and len( self.Top ) > 0 and len( self.Bottom ) > 0:
                ...
                elif self.Renderer in { "renderman", "rendermanris", "renderman22"}:
                    if len( self.Width ) > 0 and len( self.Height ) > 0:
                        ...
                        if width > 0 and height > 0:
                            ...
                            if self.Renderer == "renderman22":
                                return 'setAttr rmanGlobals.opt_cropWindowEnable 1;setAttr rmanGlobals.opt_cropWindowTopLeft %s %s; setAttr rmanGlobals.opt_cropWindowBottomRight %s %s;' % (leftPercent, topPercent, rightPercent,bottomPercent)
                            else:
    ...
    def GetMiscCommands( self ):
        if self.Renderer == "maxwell" or self.Renderer == "maxwellexport":
            ...
            return r'$options += " -rib -ribFile \\\"%s/%s\\\""; ' % ( self.GetPluginInfoEntry("RIBDirectory"), self.GetPluginInfoEntry("RIBPrefix") )
            ...
            if self.RegionRendering:
                ...
                while True:
                    renderElementNode = self.GetPluginInfoEntryWithDefault( "RenderElementNodeName" + str( renderElementIndex ), None )
                    if not renderElementNode:
                        break

                    renderElementImagePrefixEntry = "RenderElement" + str( renderElementIndex ) + "RegionPrefix"
                    if self.SingleRegionJob:
                        renderElementImagePrefixEntry += self.SingleRegionIndex

                    renderElementImagePrefix = self.GetPluginInfoEntryWithDefault( renderElementImagePrefixEntry, None )
                    redshiftMiscArgs.append( 'removeRenderLayerAdjustmentAndUnlock ' + renderElementNode + '.filePrefix; setAttr -type "string" ' + renderElementNode + '.filePrefix "' + renderElementImagePrefix + '";' )
            ...
            if len(selectedGPUs) > 0:
                ...
                self.LogInfo( "This Worker is overriding its GPU affinity, so the following GPUs will be used by Octane: " + gpus )
            ...
            if len(selectedGPUs) > 0:
                ...
                gpus = ",".join(gpuList)
                self.LogInfo( "This Worker is overriding its GPU affinity, so the following GPUs will be used by IRay: " + gpus )
    ...
    def GetRedshiftRenderCommand( self ):
    ...
    def GetRenderCommand( self ):
        if self.Renderer == "file" or (self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
            ...
            if self.RenderLayer:
                cmdArguments.append('editRenderLayerGlobals -currentRenderLayer $rl;')
            ...
            if self.Animation:
                ...
                cmdArguments.append( '$exportString = $exportString + " -sf ' +  self.StartFrame + ' -ef ' + self.EndFrame + '";')
            else:
            ...
            cmdArguments.append( 'if(`getAttr defaultArnoldRenderOptions.binaryAss` == 0){$exportString = $exportString + " -a";}' )
            cmdArguments.append( 'if(`getAttr defaultArnoldRenderOptions.outputAssBoundingBox`){$exportString = $exportString + " -bb";}' )
            cmdArguments.append( 'if(`getAttr defaultArnoldRenderOptions.expandProcedurals`){$exportString = $exportString + " -ep";}' )
            cmdArguments.append( 'if(`getAttr defaultArnoldRenderOptions.exportAllShadingGroups`){$exportString = $exportString + " -shg";	}' )
            cmdArguments.append( 'eval $exportString;' )

            return "\n".join( cmdArguments )
        elif self.Renderer == "redshift":
            return self.GetRedshiftRenderCommand()
        elif self.Renderer == "redshiftexport":
            if self.Animation:
...
class MayaBatchProcess (ManagedProcess):
    ...
    def __init__( self, deadlinePlugin, version, build, sceneFile, projectPath, startupScriptPath, renderer, delayLoadScene=False ):
        ...
        self.InitializeProcessCallback += self.InitializeProcess # pylint: disable=no-member
        self.RenderExecutableCallback += self.RenderExecutable # pylint: disable=no-member
        self.RenderArgumentCallback += self.RenderArgument # pylint: disable=no-member
    ...
    def Cleanup(self):
        ...
        del self.InitializeProcessCallback # pylint: disable=no-member
        del self.RenderExecutableCallback # pylint: disable=no-member
        del self.RenderArgumentCallback # pylint: disable=no-member
    ...
    def InitializeProcess( self ):
        ...
        self.AddStdoutHandlerCallback( "An error occurred during the Startup Script." ).HandleCallback += self.HandleFatalError
        ...
        if self.Renderer == "vray" or self.Renderer == "vrayexport":
            ...
            if self.deadlinePlugin.GetPluginInfoEntryWithDefault( "Camera", "") != "" or self.CountRenderableCameras < 1:
    ...
    def RenderExecutable( self ):
        ...
        return self.deadlinePlugin.GetRenderExecutable("RenderExecutable" + versionString, "Maya " + versionString)

    ## Called by Deadline to get the render arguments.
    def RenderArgument( self ):
        renderArguments = "-prompt"

        use_python2 = self.deadlinePlugin.GetBooleanConfigEntryWithDefault('UsePython2', False)
        if self.Version == 2022 and use_python2 and not SystemUtils.IsRunningOnMac():
            renderArguments += " -pythonver 2"
    ...
    def HandleRedshiftGPUAffinityError( self ):
        ...
        self.deadlinePlugin.FailRender( "Redshift has detected that the selected GPU '%s' is invalid. Failing the task to prevent Redshift from using all GPUs." % gpu_device_id )
